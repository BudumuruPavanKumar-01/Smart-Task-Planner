...
export class DatabaseService {
  ...
  private getRun() {
    return promisify(this.db.run.bind(this.db)) as (
      sql: string,
      params?: unknown[]
    ) => Promise<sqlite3.RunResult>;
  }
  ...
  private getGet<T>() {
    return promisify(this.db.get.bind(this.db)) as (
      sql: string,
      params?: unknown[]
    ) => Promise<T | undefined>;
  }
  ...
  private getAll<T>() {
    return promisify(this.db.all.bind(this.db)) as (
      sql: string,
      params?: unknown[]
    ) => Promise<T[]>;
  }
  ...
  async createGoal(
    ...
    await run(
      ...
      [
        ...
        goalData.deadline?.toISOString() ?? null,
        now,
        now,
        goalData.status,
      ]
    ...
    return { id, ...goalData, createdAt: new Date(now), updatedAt: new Date(now) };
  }
  ...
  async createTask(
    ...
    await run(
      ...
      [
        ...
        taskData.priority,
        taskData.status,
        taskData.startDate?.toISOString() ?? null,
        taskData.dueDate?.toISOString() ?? null,
        taskData.completedAt?.toISOString() ?? null,
        now,
        now,
      ]
    );
    ...
    return { id, ...taskData, dependencies: dependencies.map(d => d.dependsOnTaskId), createdAt: new Date(now), updatedAt: new Date(now) };
  }
  ...
  async createTaskDependency(
    ...
    await run(
      `INSERT INTO task_dependencies (id, task_id, depends_on_task_id, dependency_type, created_at) 
       VALUES (?, ?, ?, ?, ?)`,
      [id, dependencyData.taskId, dependencyData.dependsOnTaskId, dependencyData.dependencyType, now]
    );

    return { id, ...dependencyData, createdAt: new Date(now) };
  }

  async getGoalById(id: string): Promise<Goal | null> {
    const get = this.getGet<GoalRow>();
  ...
  async getTasksByGoalId(goalId: string): Promise<Task[]> {
    const all = this.getAll<TaskRow>();
    const rows = await all("SELECT * FROM tasks WHERE goal_id = ? ORDER BY created_at", [goalId]);
  ...
  async getTaskDependencies(taskId: string): Promise<TaskDependency[]> {
    const all = this.getAll<TaskDependencyRow>();
    const rows = await all("SELECT * FROM task_dependencies WHERE task_id = ?", [taskId]);
    return rows.map(row => ({
      id: row.id,
      taskId: row.task_id,
  ...
  async getTaskPlan(goalId: string): Promise<TaskPlan | null> {
    ...
    for (const task of tasks) {
      allDependencies.push(...(await this.getTaskDependencies(task.id)));
    }

    const estimatedDuration = Math.ceil(tasks.reduce((sum, t) => sum + t.estimatedHours, 0) / 8);
    const criticalPath = this.calculateCriticalPath(tasks, allDependencies);

    return { goal, tasks, dependencies: allDependencies, estimatedDuration, criticalPath, reasoning: "Task plan retrieved from database" };
  }
  ...
  private calculateCriticalPath(tasks: Task[], dependencies: TaskDependency[]): string[] {
    ...
    const visit = (taskId: string) => {
      ...
      visited.add(taskId);
      dependencies.filter(d => d.taskId === taskId).forEach(d => visit(d.dependsOnTaskId));
  ...
  async updateTaskStatus(taskId: string, status: Task["status"]): Promise<void> {
    ...
    await run("UPDATE tasks SET status = ?, completed_at = ?, updated_at = ? WHERE id = ?", [status, completedAt, now, taskId]);
  }
